...
export const VenueThemesSection = ({
  ...
  return (
    <section id="venue-themes" className="py-16 sm:py-24 bg-[#FAF7F2] border-b border-[#E5D9C8]">
            ...
            {filteredThemes.map((theme) => {
              ...
              return (
                <motion.div
                  ...
                  className="bg-white rounded-3xl border border-[#E5D9C8] overflow-hidden shadow-[0_4px_20px_rgba(80,50,30,0.04)] hover:shadow-[0_15px_35px_rgba(80,50,30,0.08)] transition-all duration-300 flex flex-col group"
                >
                  ...
                  <div className="relative aspect-[16/11] overflow-hidden bg-[#F0E6D8]">
                    ...
                    <div className="absolute top-4 left-4 flex items-center gap-2">
                      {theme.badge && (
                        <span className="px-3 py-1 rounded-full bg-white/95 backdrop-blur-md text-[10px] font-bold text-[#6B1724] uppercase tracking-widest shadow-xs">
                          {theme.badge}
                        </span>
                      )}
                      <span className="px-2.5 py-1 rounded-full bg-[#1E1B18]/70 backdrop-blur-md text-[10px] font-semibold text-white uppercase tracking-wider">
                        {theme.spaceArea}
                      </span>
                    </div>
                    ...
                    <button
                      ...
                      className="absolute top-4 right-4 w-9 h-9 rounded-full bg-white/90 backdrop-blur-md flex items-center justify-center text-[#1E1B18] hover:text-[#6B1724] transition-colors shadow-md cursor-pointer"
                    ...
                    <div className="absolute bottom-3 left-4 right-4 flex items-center justify-between text-white">
                      ...
                      <div className="flex items-center gap-1 text-xs bg-black/40 backdrop-blur-xs px-2.5 py-1 rounded-full text-[#DFB776] font-semibold">
                        <Star className="w-3.5 h-3.5 fill-[#DFB776]" />
                  ...
                  <div className="p-6 flex-1 flex flex-col justify-between space-y-4">
                    <div>
                      <h3 className="font-serif-display text-xl font-bold text-[#1E1B18] group-hover:text-[#6B1724] transition-colors leading-snug">
                        {theme.title}
                      </h3>
                      <p className="text-xs text-[#8C6D46] font-medium mt-1">
                        {theme.subtitle}
                      </p>
                      <p className="text-xs text-[#52453E] font-light mt-2 line-clamp-2 leading-relaxed">
                        {theme.description}
                      </p>
                    </div>

                    {/* Inclusions summary pills */}
                    <div className="space-y-2 pt-2 border-t border-[#EBE2D5]">
                      <span className="text-[10px] uppercase tracking-wider text-[#8C6D46] font-bold block">
                        Signature Highlights:
                      </span>
                      <ul className="text-xs text-[#52453E] space-y-1.5">
                        {theme.inclusions.slice(0, 3).map((inc, i) => (
                          <li key={i} className="flex items-start gap-2 text-[11px]">
                            <Check className="w-3 h-3 text-[#6B1724] flex-shrink-0 mt-0.5" />
                    ...
                    {/* Action Buttons */}
                    <div className="pt-3 border-t border-[#EBE2D5] flex items-center gap-2.5">
                      <button
