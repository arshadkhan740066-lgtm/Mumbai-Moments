...
export const LookbookSection = ({ onOpenInquiry }: LookbookSectionProps) => {
  return (
    <section id="lookbook" className="py-16 sm:py-24 bg-[#FFFDFB] border-b border-[#E8D9DC]">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center max-w-3xl mx-auto mb-12 sm:mb-16">
          <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-white border border-[#E8D9DC] text-[#8F3F50] text-xs font-semibold uppercase tracking-widest mb-3 shadow-xs">
            <Heart className="w-3.5 h-3.5 text-[#8F3F50]" />
          ...
          {REAL_CLIENT_TRANSFORMATIONS.map((story) => (
            ...
            >
              <div className="relative aspect-[16/11] overflow-hidden bg-[#F8EDEF]">
                ...
                <div className="absolute top-4 left-4 px-3 py-1 rounded-full bg-white/95 text-[10px] uppercase tracking-widest text-[#8F3F50] font-bold shadow-xs">
                  {story.occasion}
                </div>

                <div className="absolute bottom-3 left-4 right-4 text-white">
                  <div className="text-[10px] text-[#C9A15B] uppercase tracking-widest font-semibold">
                    {story.spaceUsed}
                  </div>
              ...
              <div className="p-6 flex-1 flex flex-col justify-between space-y-4">
                <div className="space-y-3">
                  <div className="relative pl-6 text-xs text-[#241E20] font-serif-display italic font-semibold leading-relaxed">
                    <Quote className="w-4 h-4 text-[#C9A15B] absolute top-0 left-0" />
                    "{story.quote}"
                  </div>
                ...
                <div className="pt-3 border-t border-[#E8D9DC]">
                  <span className="text-[10px] uppercase tracking-wider text-[#C9A15B] font-bold block mb-1.5">
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {story.addOnsIncluded.map((add, i) => (
                      <span
                        key={i}
                        className="px-2.5 py-1 rounded-full bg-[#FFFDFB] border border-[#E8D9DC] text-[10px] font-medium text-[#8F3F50]"
                      >
                        {add}
                      </span>
                    ))}
        ...
        {/* FAQs Section */}
        <div className="bg-white border border-[#E8D9DC] rounded-3xl p-8 sm:p-12 shadow-[0_10px_35px_rgba(80,50,30,0.04)] max-w-4xl mx-auto">
          <div className="text-center max-w-2xl mx-auto mb-10">
            <div className="inline-flex items-center gap-1.5 text-xs uppercase tracking-widest text-[#C9A15B] font-bold mb-2">
          ...
          <div className="space-y-6">
            {FAQ_LIST.map((faq, i) => (
              <div key={i} className="pb-6 border-b border-[#E8D9DC] last:border-none last:pb-0">
                <h4 className="font-serif-display text-base sm:text-lg font-bold text-[#241E20] mb-2 flex items-start gap-2">
                  <span className="text-[#8F3F50] font-normal">Q:</span>
                  <span>{faq.q}</span>
                </h4>
                <p className="text-xs sm:text-sm text-[#5C4E52] font-light leading-relaxed pl-5">
                  {faq.a}
                </p>
              </div>
            ))}
          ...
          <div className="mt-8 pt-6 border-t border-[#E8D9DC] text-center">
            <p className="text-xs text-[#5C4E52] mb-3">
