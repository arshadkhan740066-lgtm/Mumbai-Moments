...
export const SpacesPage = ({ onOpenInquiry }: SpacesPageProps) => {
  ...
  return (
    <div className="min-h-screen bg-[#FAF7F2]">
      {/* Hero */}
      <section className="relative pt-28 pb-16 sm:pt-36 sm:pb-20 bg-gradient-to-b from-[#1E160E] via-[#2A1C12] to-[#FAF7F2] overflow-hidden">
        ...
        <div className="absolute inset-0 bg-gradient-to-b from-[#1E160E]/90 to-[#FAF7F2]" />
              ...
              <button
                ...
                className={`px-4 py-2 rounded-full text-xs font-semibold uppercase tracking-wider transition-all cursor-pointer ${
                  selectedSpace === space.id
                    ? 'bg-[#C59A5D] text-[#18140E] border-2 border-[#C59A5D]'
      ...
      {/* Property Inclusions */}
      <section className="py-10 border-b border-[#E5D9C8] bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex flex-wrap justify-center gap-3">
            {VENUE_PROPERTY_INFO.inclusionsStandard.map((item, i) => (
              <div key={i} className="flex items-center gap-2 px-4 py-2 rounded-full bg-[#FAF5EE] border border-[#E5D9C8]">
                <Check className="w-3.5 h-3.5 text-[#6B1724] flex-shrink-0" />
                <span className="text-xs font-medium text-[#382E28]">{item}</span>
              </div>
            ))}
          </div>
        </div>
            ...
            >
              ...
              <div className={`relative rounded-3xl overflow-hidden aspect-[4/3] shadow-2xl ${idx % 2 === 1 ? 'lg:order-2' : ''}`}>
                ...
                <div className="absolute top-4 left-4">
                  <span className="px-3 py-1.5 rounded-full bg-white/95 text-[10px] font-bold uppercase tracking-widest text-[#6B1724] shadow-md">
                    100% Private
                  </span>
                </div>
                <div className="absolute bottom-4 left-4 right-4">
                  <p className="text-[#DFB776] text-[10px] uppercase tracking-widest font-bold">{space.capacity}</p>
              ...
              <div className={`space-y-6 ${idx % 2 === 1 ? 'lg:order-1' : ''}`}>
                <div>
                  <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-[#FAF5EE] border border-[#DFCDB7] text-[#8C6D46] text-[10px] font-bold uppercase tracking-widest mb-3">
                    <ShieldCheck className="w-3 h-3" />
                    <span>{space.privacyLevel}</span>
                  </div>
                  <h2 className="font-serif-display text-3xl sm:text-4xl font-bold text-[#181310] mb-2">{space.name}</h2>
                  <p className="font-cursive text-xl text-[#C59A5D]">{space.tagline}</p>
                </div>

                <p className="text-sm text-[#52453E] font-light leading-relaxed">{space.description}</p>
                ...
                <div className="space-y-2.5">
                  ...
                  <ul className="space-y-2">
                    {space.keyFeatures.map((feature, fi) => (
                      <li key={fi} className="flex items-start gap-2.5 text-sm text-[#382E28]">
                        <Star className="w-3.5 h-3.5 text-[#C59A5D] fill-[#C59A5D] flex-shrink-0 mt-0.5" />
                        <span>{feature}</span>
                      </li>
                    ))}
                  </ul>
      ...
      <section className="py-16 sm:py-20 bg-gradient-to-br from-[#6B1724] to-[#3A0B12] text-white border-t border-[#E5D9C8]">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center space-y-6">
          <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-white/15 border border-white/25 text-[#DFB776] text-xs font-bold uppercase tracking-widest">
            <Heart className="w-3.5 h-3.5 fill-[#DFB776]" />
          ...
          <button
            onClick={onOpenInquiry}
            className="inline-flex items-center gap-2 px-8 py-4 rounded-full bg-[#DFB776] text-[#18140E] text-sm font-bold uppercase tracking-widest shadow-xl hover:brightness-105 transition-all cursor-pointer"
          >
